...
class TFLogger(Node):
    def __init__(self):
        ...
        # Full trajectory recorded only after robot motion is confirmed.
        self.declare_parameter(
            "output_file",
            os.path.expanduser(
                "~/Go2_Walk_Base_Data_Sensor/Single_synchronized_transforms.csv"
            ),
        )

        ##-- Filtered output file path where the filtered data gets stored --##
        self.declare_parameter(
            "filtered_output_file",
            os.path.expanduser(
                "~/Go2_Walk_Base_Data_Sensor/"
                "Single_synchronized_transforms_filtered.csv"
            ),
        )
                ##-- declaring the path where start and stop timestamp gets stored --##
        self.declare_parameter(
            "command_event_file",
            os.path.expanduser(
                "~/Go2_Walk_Base_Data_Sensor/"
        ...
        self.command_event_file = os.path.expanduser(
            self.get_parameter("command_event_file")
            .get_parameter_value()
            .string_value
        )

        self.filtered_output_file = os.path.expanduser(
            self.get_parameter("filtered_output_file")
            .get_parameter_value()
            .string_value
        )
    ...
    def initialize_output_file(self):
        ...
        ##-- Create a fresh filtered CSV when the TF logger starts --##
        try:
            if os.path.isfile(self.filtered_output_file):
                os.remove(self.filtered_output_file)
        except OSError as error:
            self.get_logger().warn(
                "Could not remove the previous filtered CSV: "
                f"{error}"
            )
    ...
    def filter_rows_to_first_stationary_timestamp(self):
        ...
        # Preserve the original 10-second filtered CSV and create
        # a second CSV ending at the stationary timestamp.
        output_root, output_extension = os.path.splitext(
            self.filtered_output_file
        )
        ##--- if the original file has no extension, default to .csv ---##
        if not output_extension:
            output_extension = ".csv"
        ##-- Builds the new filename by appending _stationary before the extension--##
        stationary_output_file = (
            f"{output_root}_stationary{output_extension}"
        )

        # Remove a stationary-filtered CSV left behind by an older trial.
        ##--- Checking whether a previous version of the stationary-filtered CSV exists and removing it if it does ---##
        try:
            if os.path.isfile(stationary_output_file):
                os.remove(stationary_output_file)

        except OSError as error:
            self.get_logger().warn(
                "Could not remove the previous stationary-filtered CSV: "
                f"{error}"
            )
        ...
        try:
            with open(
                ...
                for row in reader:
                    ...
                    # If multiple MOVE_START rows exist, the final
