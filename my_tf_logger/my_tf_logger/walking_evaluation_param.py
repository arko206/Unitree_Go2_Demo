...
def main():
    ...
    diff_act_dir = "/home/arka/Demo_Report_Go2/Diffusion_iter_200/Learning_Walk_200/Diff_200_learn_200"
    ...
    param_vec = np.array([0.241777, 0.0, 0.0], dtype=np.float32)

    # -------- EVALUATION LOOP (single run) --------
    ...
    with tqdm(total=max_steps, desc="Unitree Walk Traj_Eval") as pbar:
        while not done:
            B = 1
            # (1) stack last obs_horizon observations: (2, 6)
            obs_seq = np.stack(obs_deque)
            print(f"Before normalizing, obs at step {step_idx} is:", obs_seq)
            ...
            print("The parameter sequence is:", param_seq)
            ...
            print("After normalizing, nobs shape:", nobs_t.shape)
            print("After normalizing, nparam shape:", nparam_t.shape)
            ...
            with torch.no_grad():
                ...
                print("Timesteps in noise scheduler:", noise_scheduler.timesteps)
            ...
            print(f"The Action taken at step {step_idx} is:", action)

            array_action = np.array(action, dtype=np.float32)
            print("All actions taken so far in this step:", array_action)

            # update obs_deque with last action as next “pose”
            obs_deque.append(array_action[-1])
            print("Updated Observation Queue:", obs_deque)
    ...
    import json
    iter_id = 1  # single evaluation

    fwd_action_list_as_lists = [arr.tolist() for arr in fwd_action_list]
    with open(os.path.join(base_act_dir, f"Walk_200_diffusion_200_actions_{iter_id}.json"), "w") as f:
        json.dump(fwd_action_list_as_lists, f)

    observations_as_lists = [obs.tolist() for obs in obs_list]
    with open(os.path.join(base_obs_dir, f"Walk_200_diffusion_200_observations_{iter_id}.json"), "w") as f:
        json.dump(observations_as_lists, f)

    with open(os.path.join(diff_act_dir, f"Walk_200_diffusion_200_actions_{iter_id}.txt"), "w") as f:
        decimals = 6
        fmt = f"{{:.{decimals}f}}"
        rows = []
    ...
    print("Walking diffusion evaluation finished.")
