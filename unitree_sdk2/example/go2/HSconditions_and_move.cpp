...
public:
  ...
  void RobotControl()
  {
    ...
    if (std::isfinite(front_local) && (front_local >= 2.0) && (front_local <= 9.0)) {
      if (!moving_started_) {
          moving_started_ = true;
          std::cout << "[INFO] Starting forward motion..." << std::endl;
      }
      std::cout << "Start Moving the Robot"<< std::endl;
      sport_client.Move(0.2, 0.0, 0.0);
      return;
   }
   else if (std::isfinite(front_local) && (front_local >= 0) && (front_local <= 2.0)){
    ...
    // if there are valid detections, increase the count. We require a few consecutive detections to trigger the jump to avoid noise.
      ...
      if (valid_detect_count_ >= required_consecutive) {
        std::cout << "[INFO] Obstacle condition satisfied. Stopping and triggering Handstand..."
                  << std::endl;

        std::cout << "Handstand..." << std::endl;
        jump_triggered_ = true;

        sport_client.Move(0.0, 0.0, 0.0); // stop before jumping
        usleep(1000000); 
        sport_client.FreeWalk(); // ensure stop command is processed
        usleep(5000000); // brief pause to ensure static walk is processed
        std::cout << "[INFO] Sending Handstand command now..." << std::endl;
        sport_client.HandStand(true);
        sleep(10); // hold handstand for 10 seconds
        sport_client.Move(0.1, 0.0, 0.0);
        sleep(7);
        sport_client.HandStand(false); // come down from handstand
        sleep(5);
        sport_client.BalanceStand();
        std::cout << "[INFO] Handstand command sent." << std::endl;
      ...
      std::cout << "Robot is not going to move." << moving_started_ << std::endl;
    }
